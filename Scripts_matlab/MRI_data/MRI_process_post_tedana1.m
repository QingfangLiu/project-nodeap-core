...
% This code continues processing rt-fMRI data on optimally combined data
% from tedana output 'desc-optcom_bold.nii.gz' (not denoised)
...
for r = 1:n_rest_names % to loop over rest_names
    ...
    if ~isempty(dir(fullfile(curr_tedana_dir,'s6w2desc-optcom_bold.nii'))) && ...
        ~isempty(dir(fullfile(curr_tedana_dir,'s6w3desc-optcom_bold.nii'))) && ...
        ~isempty(dir(fullfile(curr_tedana_dir,'s6desc-optcom_bold.nii')))
    ...
    func_files = fullfile(curr_tedana_dir,'desc-optcom_bold.nii');
    if ~exist(func_files,'file')
        func_files_gz = fullfile(curr_tedana_dir,'desc-optcom_bold.nii.gz');
...
if isempty(dir(fullfile(curr_tedana_dir,'s6w2desc-optcom_bold.nii')))
    ...
    % Run preprocessing
    job_name = fullfile(jobdir, sprintf('Post_tedana1_Processing_%s.mat',curr_rest));
...
if isempty(dir(fullfile(curr_tedana_dir,'s6w3desc-optcom_bold.nii')))
    matlabbatch = [];
    % Normalise write
    fname = fullfile(niidir,'anat', sprintf('y_%s', aname(1).name));
    ...
    % Run preprocessing
    job_name = fullfile(jobdir, sprintf('Post_tedana1_Processing2_%s.mat',curr_rest));
    %save(job_name, 'matlabbatch');
    %spm_jobman('run', matlabbatch)
    clear matlabbatch
end
...
if isempty(dir(fullfile(curr_tedana_dir,'s6desc-optcom_bold.nii')))
    ...
    % Run preprocessing
    job_name = fullfile(jobdir, sprintf('Post_tedana1_Processing3_%s.mat',curr_rest));
    save(job_name, 'matlabbatch');
    spm_jobman('run', matlabbatch)
    clear matlabbatch
