...
% rerun this using the LPFC clusters as the seed region
...
if subj==18 && r==7 
    fnames = dir(fullfile(functdir,'s6rf*.nii')); % this session went through another coregistration to match dim
end
...
for i=1:length(regions)
    ...
    % make GLM folder
    modeldir = fullfile(func_conn_dir,sprintf('GLM_%s',regions(i).targetName));
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess.regress.name = regions(i).targetName;
    ...
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.name = regions(i).targetName;
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.weights = 1;
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.sessrep = 'repl';
    matlabbatch{3}.spm.stats.con.delete = 0;
    
    fname = fullfile(jobdir, sprintf('FuncConn_GLM_%s_%s_PAID',regions(i).targetName,curr_rest));
