...
SubID = Subs{j};
if strcmp(SubID,'NODEAP_17') % Skip the current iteration when SubID is NODEAP_17
   disp('Skipping iteration for NODEAP_17');
   continue;
end
...
all_df = [];
for i = 1:3
    x = res.pleasantness_ratings_PRE{1,i};
    idx1 = find(x(:,1)==1);
    idx2 = find(x(:,1)==2);
...
for i = 1:3
    x = res.pleasantness_ratings_POST{1,i};
    idx1 = find(x(:,1)==1);
    idx2 = find(x(:,1)==2);
    mean_ratings = [[1;2],[mean(x(idx1,5));mean(x(idx2,5))]];
...
for i = 1:3
    for k = 1:5 % 5 runs in conditioning
        ...
        load(fullfile(SubDir,tmp_mat_name))
        
        if isempty(x) % if x is empty then use conditioning as x
            x = conditioning; % used for NODEAP_59, Sess 2, Run 1
        else % check if x and conditioning matrix are the same
            if ~isequaln(x,conditioning)
...
if if_process_pleasant_data
    ...
    writetable(AllData, '../ProcessedData/Pleasant.xlsx')
end
...
if if_process_choice_data
    ...
    writetable(AllChoiceData, '../ProcessedData/Choices.xlsx')
end
...
if if_process_conditioning_data
    ...
    writetable(AllConditioningData, '../ProcessedData/Conditioning.xlsx')
end
