...
% updated from a similar version to use the aal atalas that TK sent
% with fewer ROIs than the one I used before
...
aal_path = fullfile(maskpath,'TKsent','raal.nii'); % use resliced aal atlas from TK
aal_img = spm_vol(aal_path);
aal_data = spm_read_vols(aal_img);
num_rois = max(aal_data(:)); % Total number of ROIs=116 (from 170)

gm_nii = fullfile(maskpath,'gm_0.1_2mm.nii'); % gray matter mask
gm_dat = spm_read_vols(spm_vol(gm_nii));
gm_idx = find(gm_dat > 0);
n_gm_voxels = length(gm_idx); % number of gray matter voxels in 2mm
...
for r = 1:n_rest_names 
        ...
        if include_shift
            mat_name = fullfile(func_conn_dir,sprintf('conn_matrix_%s_w_shift_new.mat',curr_rest));
        else
            mat_name = fullfile(func_conn_dir,sprintf('conn_matrix_%s_new.mat',curr_rest));
        end
