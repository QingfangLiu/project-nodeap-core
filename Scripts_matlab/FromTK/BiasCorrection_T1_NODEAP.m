...
dcm2niipath = 'C:\software\MRIcroGL\Resources\dcm2niix';

studydir = 'C:\Data\NODEAP\';
sn = 'NODEAP_pilot1';
...
if do_dicom_import
    ...
    % convert T1 file
    outdir = fullfile(subdir,'nii', 'anat');
    if ~exist(outdir,'dir')
        mkdir(outdir);
    end
    unix(sprintf('%s -o %s %s', dcm2niipath, outdir, anatpath))
    
    
    % convert resting-state data
    outdir = fullfile(subdir,'nii', 'day0_me');
    if ~exist(outdir,'dir')
        mkdir(outdir);
    end
    unix(sprintf('%s -o %s %s', dcm2niipath, outdir, functpath))
...
if do_bias_correction
    ...
    subfolder = fullfile(subdir, 'nii', 'anat'); % path with anatomical data
