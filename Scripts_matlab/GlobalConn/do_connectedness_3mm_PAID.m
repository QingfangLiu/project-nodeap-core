...
% note: 9/18
% I added this option, but have not run with the shift yet
% to change to data saving structure if want to use it
...
for subj = 1:nSubIDlist
    ...
    respath = fullfile(studydir, 'GlobalConn_PAID', SubID, Option);
    ...
    for r = 1:n_rest_names % to loop over rest_names
        ...
        if ~exist(nhdr.fname,'file') % check if it has been created
            ...
            if include_shift
                use_name = 'tc_filtered_3mm_w_shift.mat';
            else
                use_name = 'tc_filtered_3mm.mat';
            end
                
            load(fullfile(studydir, 'GlobalConn_PAID', SubID, curr_rest, use_name)); 
...
if agg_contrast_map
    ...
    pathname = fullfile(studydir, 'GlobalConn_PAID', 'Agg', Option, contrast_names{c});
    ...
    for subj = 1:nSubIDlist
        ...
        ffiles{subj} = fullfile(studydir, 'GlobalConn_PAID', SubID, Option, filename);
    end
...
if agg_contrast_map
    ...
    roi = regions{region};
    % where to save aggregate contrast map results
    pathname = fullfile(studydir, 'GlobalConn_PAID', 'Agg', Option, sprintf('%s_%s',contrast_names{c},roi));
    if ~exist(pathname, 'dir')
        mkdir(pathname);
    end
    ...
    for subj = find(tmploc==1)'
        ...
        filename = sprintf('%s.nii', contrast_names{c});
        filename = sprintf('s6%s,1',filename); % use smoothed images
        ffiles{subctr} = fullfile(studydir, 'GlobalConn_PAID', SubID, Option, filename);
    end 
