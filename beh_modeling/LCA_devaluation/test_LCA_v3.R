...
save.image(file = 'test_LCA_v3.RData')


# load saved data
load('test_LCA_v3.RData')
par(mfrow=c(1,2))
...
p1=ggplot(mat_long, aes(x = Time, y = Value, color = Option)) +
  ...
  annotate("segment",x = 600, xend = 600, y = 2.8, yend = 1.8,  
...
p2=ggplot(mat_long, aes(x = Time, y = Value, color = Option)) +
    ...
    scale_linetype_manual(values = c('Non-sated'="solid", 'Sated'="dotted")) +
    scale_color_manual(values = c('Sated'='gray','Non-sated'='#832a78')) +
    annotate("segment",x = 0, xend = 530, y = 0, yend = 3,  
           arrow = arrow(type = "closed", length = unit(0.2, "cm")),
           color = "#832a78", size = 1) +
    annotate("segment",x = 0, xend = 600, y = 0, yend = 2.2,  
           arrow = arrow(type = "closed", length = unit(0.2, "cm")),
           color = "gray", size = 1) +
    common +
    theme(legend.position = c(0.75, 0.2))

pdf('LCA_illustrations_v3.pdf',8,4)
