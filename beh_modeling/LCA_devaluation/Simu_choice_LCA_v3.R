...
n_steps <- 3000        # Time steps for accumulation process
  ...
  for (j in 1:length(S_grid)) {
    ...
    for (sim in 1:n_simulations) {
      ...
      for (trial in 1:n_trials) {
        simu <- func_LCA(S1, S2, bias1, bias2, gamma, beta_1, beta_2, 
                           sigma, dt, threshold, eps, t0)
        choices[trial] <- simu$choice
      }
      # Accumulate choices and correct choices
      total_choices <- total_choices + mean(choices,na.rm=T)  # Average of choices (proportion choosing option 1)
    }
...
pdf('Simu_LCA_with_lateral_inhibition_eps_v3.pdf',13,12)
...
saveRDS(choice_outcomes_df_with_betas,
        file = 'Simu_LCA_strength_inhibition_eps_v3.rds')
...
paras = expand.grid(bias2 = c(0,0.15,0.3),eps = seq(0.5,1,0.25))
choice_outcomes_df_with_bias = NULL

for(b in 1:nrow(paras)){
  eps <- paras$eps[b]       # precision of identity knowledge
  bias1 = 0
  bias2 <- paras$bias2[b]   # bias towards non-sated option 
...
for(b in 1:nrow(paras)){
  p[[b]] = ggplot(choice_outcomes_df_with_bias[[b]], 
    ...
    scale_fill_gradient2(low = "red", high = "green", 
                         mid = 'white', midpoint = 0.5,
                         name = "P (sated odor)") +
    labs(title = paste('eps =',paras$eps[b],'bias2 =',paras$bias2[b]), 
         x = xlab, y = ylab)
}

saveRDS(choice_outcomes_df_with_bias,file = 'Simu_LCA_strength_bias_eps_v3.rds')
pdf('Simu_LCA_with_bias_eps_v3.pdf',13,12)
ggarrange(p[[1]],p[[2]],p[[3]],
          p[[4]],p[[5]],p[[6]],
          p[[7]],p[[8]],p[[9]],
          common.legend = T,legend = 'right',
