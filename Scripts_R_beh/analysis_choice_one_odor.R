...
summary_choice_one_more = choice_dat_one %>%
  group_by(SubID,StimLoc,StimOrder,StimOrder_day1,StimOrder_day2,
           Cond,Sess,PrePost,Set,OdorType) %>%
  reframe(Choice=mean(ChosenOdor,na.rm = T)) 

pre_choice_A = subset(summary_choice_one_more,
                      Set=='A' & PrePost=='Pre')$Choice
post_choice_A = subset(summary_choice_one_more,
                       Set=='A' & PrePost=='Post')$Choice
post_choice_B = subset(summary_choice_one_more,
...
# containing choice update for each session
summary_choice_corrected = summary_choice_one_more %>%
