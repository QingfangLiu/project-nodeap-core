...
% conduct functional conn analysis using data from tedana and other
% following analyses
% do this for rs-fMRI data after each TMS
% using data in native space
% opt-com but not denoised data
...
for r = 1:n_rest_names % to loop over rest_names
        ...
        fprintf('%s of %s\n',curr_rest,subdir)
        functdir = fullfile(subdir,'tedana_output',curr_rest,'aic'); % path to rsfRMI images
        func_conn_dir = fullfile(subdir, 'FuncConn_tedana1',curr_rest); % path to save the GLM result from tedana result
...
fnames = dir(fullfile(functdir,'s6desc-optcom_bold.nii')); % smoothed data in native space
...
for i=1:length(regions)
    ...
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.weights = [-1;1;zeros(6,1)];
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.sessrep = 'repl';
    matlabbatch{3}.spm.stats.con.delete = 0;
    
    fname = fullfile(jobdir, sprintf('FuncConn_GLM_%s_%s_tedana',regions(i).seedName,curr_rest));
